# Chapter 13 instructor guide — Can a passing candidate activate after its context changed?

**Created:** 2026-09-08 · **Last-updated:** 2026-09-08 · **Status:** DRAFT educator companion v1

## Learning outcomes

By the end, the student can implement the stated decision contract, explain a failing shortcut with a concrete counterexample, trace the related decision through the chapter runtime, and state the limits of the observed evidence. The complete chapter construction remains in [Improve behavior with evaluated changes](https://www.profrod.ai/book/ch13-improvement); this lab isolates one decision for independent work and then reconnects it to that implementation.

## Preparation and classroom setup

Read the chapter and its checkpoint before teaching. Prerequisites: Chapter 12 evaluation; versioned configuration and compare-and-swap. Use the cumulative Python 3.14 environment and repository setup from the book conventions. Open `ch13-improvement-class-v1.ipynb` in an existing notebook environment whose interpreter can import the book dependencies. The lab locates the checkout from the working directory or `SOVEREIGN_AGENT_REPO`; it checks the checkpoint SHA-256 `02f340da68541a9b38062fb781c77290fb4cbf0a7c109bfcec88ae64c0e086cd` before executing it. It never installs dependencies for students. If the kernel is older or the checkpoint differs, correct the environment or use the matching lesson version; do not delete the guard.

If a Python 3.14 notebook kernel is unavailable, use the included standard-library runner from the repository root: `uv run --python 3.14 python book/always_on/educator/run_lesson_v1.py --chapter 13 --output /tmp/lucy-ch13-class.json`. The output path must be new. Read the notebook cells in an editor, implement `decide` there and rerun; the runner executes those saved cells and records submission results separately from the answer key. It does not install Jupyter or substitute another Python interpreter.
...
A candidate passes its cases at configuration generation five. Another skill activates while the test is running and generation becomes six. Should the original report still authorize activation?

Implement decide(case). Failed checks return REJECTED. Otherwise an evaluated_generation unequal to current_generation returns STALE. Otherwise return ACTIVATE. The caller must compare and update atomically; this pure function supplies only the decision.
...
def worked_decide(case):
    if not case["passed"]:
        return "REJECTED"
    if case["evaluated_generation"] != case["current_generation"]:
        return "STALE"
    return "ACTIVATE"
```

The notebook supplies 4 independent initial cases. Inspect their literal expected outcomes, not just the final assertion. Input mutation is a failure even if the returned value happens to match. The tempting shortcut is `lambda case: 'ACTIVATE' if case['passed'] else 'REJECTED'`; its failing cases are printed separately. A student who merely pastes this answer still needs to explain the rejected case, implement the transfer, and trace the runtime.

## Runtime connection

checkpoints/ch13.py: FollowsCandidate, ConcurrentChange and change_skill calls. Inspect retained reports, feedback events and reopened active versions.

Expected observations include: `Configuration changes during evaluation: STALE`, `Active configuration survives reopen: True`. These strings help locate the result; the checkpoint itself exercises the underlying behavior and assertions. Ask the student to identify the real state or tool result behind each observation. Printing the same words from a new stub would not meet the lab outcome.

## Transfer task and worked discussion

Find the candidate that changes GBP to euros and the concurrently activated reporting skill. Explain why rollback also needs evaluation and provenance rather than deleting the later version.

Currency regression is REJECTED while version1 stays active. Version3 activates and version1 can be selected again as ROLLED_BACK; historical rows remain. The concurrent candidate is STALE despite passing checks, because those checks describe an older configuration.
...
Learning from feedback here changes external guidance, not model weights. Passing a finite scenario suite does not grant extra authority.
...
The exercise is not a safe activation transaction by itself. The cumulative change_skill path persists versions and rechecks configuration under its database boundary.
