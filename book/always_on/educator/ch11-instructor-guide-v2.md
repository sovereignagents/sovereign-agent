...
Supersedes v1 for new classes. Use `ch11-isolation-class-v2.ipynb`. Preserve first attempts and the distributed v1 files. The book remains at profrod.ai/book.

## Why Lucy needs this lesson

Private shop data can leave a registered handler despite Lucy withholding permission to use it.

The student should finish able to explain that consequence using an actual altered implementation, a retained observation and a repair. The ten-minute decision function is a warm-up. The central practical copies and changes `src/sovereign_agent/tool_dispatch.py`; executing an untouched checkpoint is not completion of that practical.

## Prepare the actual experiment

Prerequisites: Chapter 10 write boundaries; external documents as data and capability sets. Use Python 3.14 with the book’s locked dependencies. The current educator index names the exact cohort commit, and the runtime experiment verifies SHA-256 `02e289df4c3f913f1cf9a1fa6267cd53e8845aa55235164097e67f207fe49134` before copying the target. Do not remove a mismatched-hash guard; locate the pinned files. Chapter 1 alone remains standalone standard-library Python 3.11 or newer.
...
The optional whole-chapter checkpoint is a separate cell with RUN_FULL_CHECKPOINT=False. Its ceiling is 180 seconds. Read its process behavior before enabling it: checkpoints/ch11.py: CompromisedModel, main and optional_tools. Trace supplier_document → hostile request → tool_not_allowed, then the MCP catalog response. The source probe and checkpoint differ in scope; retain their results under separate labels. Specifically, Chapter 10 and 16 checkpoints kill an actual local worker; Chapter 11 runs an MCP child but does not run containers by default; Chapter 15 does not install a service or reboot a host.
...
Target: `src/sovereign_agent/tool_dispatch.py:57`. The notebook prints surrounding lines so students can check this location against the pinned file.
...
The independently authored expected baseline is `{"private_handler_calls": 0}`. The expected broken result is `{"private_handler_calls": 1}`. A broken trial marked PASS means the fault behaved as predicted; it does not mean the modified runtime is safe.

A forced call tests enforcement even if a good model refuses hostile prose. The tool must be registered but disallowed to isolate this check. An unregistered tool would still be refused after the mutation.
...
Try an unregistered name, then invalid arguments for an allowed registered tool. Explain which independent checks remain. A process boundary still needs separate containment evidence.

If no unauthorized call occurs after removing allowed, check that the tool really is registered. Missing registration is another refusal path and can mask the tested flaw. Use the private read-only fixture so a forced compromised-model request has a visible handler call without any purchase. A child process is not OS containment.
