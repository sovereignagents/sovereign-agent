...
The notebook supplies 6 independent initial cases. Inspect their literal expected outcomes, not just the final assertion. Input mutation is a failure even if the returned value happens to match. The tempting shortcut is `lambda case: {'tubs': case['guests'] // 10, 'total_pence': (case['guests'] // 10) * 500, 'model_calls': 0}`; its failing cases are printed separately. A student who merely pastes this answer still needs to explain the rejected case, implement the transfer, and trace the runtime.

## Runtime connection

checkpoints/ch14.py: quote boundaries, WaitingModel, cancellation and reserve_model_call on a replacement claim.

Expected observations include: `Replacement cannot reset assignment allowance: True`, `Architecture decision: retain the function`. These strings help locate the result; the checkpoint itself exercises the underlying behavior and assertions. Ask the student to identify the real state or tool result behind each observation. Printing the same words from a new stub would not meet the lab outcome.

## Transfer task and worked discussion

Add cases for one and 200 guests. In the process experiment, compare quote quality and calls, then cancel a parent while the research worker waits. What happens to completed stock work?

The endpoints need one and twenty tubs, costing500 and10000. The fixed quote has no demonstrated quality gain from a model. The real experiment permits overlapping work, cancellation prevents the child’s late result from acquiring authority, and the completed parent stock result remains DONE. Assignment budgets survive replacement.
...
Concurrency is not automatically useful intelligence. A research quote is not a stock reservation or purchasing grant.
