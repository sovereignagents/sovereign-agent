...
Supersedes v1 for new classes. Use `ch13-improvement-class-v2.ipynb`. Preserve first attempts and the distributed v1 files. The book remains at profrod.ai/book.
...
The optional whole-chapter checkpoint is a separate cell with RUN_FULL_CHECKPOINT=False. Its ceiling is 180 seconds. Read its process behavior before enabling it: checkpoints/ch13.py: FollowsCandidate, ConcurrentChange and change_skill calls. Inspect retained reports, feedback events and reopened active versions. The source probe and checkpoint differ in scope; retain their results under separate labels. Specifically, Chapter 10 and 16 checkpoints kill an actual local worker; Chapter 11 runs an MCP child but does not run containers by default; Chapter 15 does not install a service or reboot a host.
...
Target: `src/sovereign_agent/assistant_context.py:174`. The notebook prints surrounding lines so students can check this location against the pinned file.
...
The independently authored expected baseline is `{"stale_candidate_activated": false, "active_names": ["reporting"]}`. The expected broken result is `{"stale_candidate_activated": true, "active_names": ["opening", "reporting"]}`. A broken trial marked PASS means the fault behaved as predicted; it does not mean the modified runtime is safe.

The callback activates a different skill through the real activation API while the candidate is being evaluated. Named True results are a deterministic concurrency fixture, not evidence of model quality.
...
Currency regression is REJECTED while version 1 stays active. Version 3 activates and version 1 can be selected again as ROLLED_BACK; historical rows remain. The concurrent candidate is STALE despite passing checks, because those checks describe an older configuration.

## Transfer and chapter-specific remediation

Run a candidate with a named False result: the evaluation guard must refuse it before the stale-state comparison. Find version 3 in retained activation data in the full checkpoint; its printed status omits the version.

If the learner compares generations with less-than, use an evaluated generation greater than current. The contract requires equality in either direction. The real runtime uses a configuration digest, not this toy integer. In the full checkpoint, Passing candidate prints ACTIVATED but not version 3: inspect the good report and assistant.skill.activated event/version rows immediately before rollback.
