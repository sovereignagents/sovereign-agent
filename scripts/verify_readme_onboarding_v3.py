...
`git archive` extraction of the staged prospective commit, not a reused working
tree), points `uv` at a fresh
...
def _clean_worktree_copy(dest: Path) -> None:
    """Archive the staged tree so pre-commit verification tests the proposed bytes.

    An unstaged tracked edit is a mismatch, not permission to test another tree.
    Untracked files are deliberately excluded: stage explicit reviewed paths first.
    """
    ...
    archive = subprocess.run(["git", "archive", tree], cwd=ROOT, check=True, capture_output=True)
